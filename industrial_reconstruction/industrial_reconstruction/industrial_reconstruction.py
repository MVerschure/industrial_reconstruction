    ...
    def cameraCallback(self, depth_image_msg, rgb_image_msg):
        if self.record:
            ...
            try:
                ...
                cv2_gray_img = self.bridge.imgmsg_to_cv2(rgb_image_msg, rgb_image_msg.encoding)
                cv2_rgb_img = cv2.cvtColor(cv2_gray_img, cv2.COLOR_GRAY2RGB)
                ...
                if (tran_dist >= self.translation_distance) or (rot_dist >= self.rotational_distance):
                    ...
                    if self.live_integration and self.tsdf_volume is not None:
                        ...
                        try:
                            ...
                            self.get_logger().info("Integrated frame " + str(self.processed_frame_count))
                            self.integration_done = True
